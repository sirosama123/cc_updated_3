...
class tasks extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final stream1 = FirebaseFirestore.instance.collection('vendors').doc(Provider11.uid).collection("tasks").snapshots();
            ...
            return  Padding(
              ...
              child: Container(
                child:GestureDetector(
                  onTap: (){
                    print(data['all_tasks_uid']);
                     Navigator.push(
                              context,
                              MaterialPageRoute(builder: (context) => JobDetails2(
                                amount: data['amount'], 
                                datee: data['date'].toString(),
                                description: data['description'],
                                jobType: data['jobTask'], 
                                serviceType: data['jobTask'],
                                status: data['status'], 
                  ...
                  child:  MyJobRect(jobtype: data['jobTask'], name: data['name'], servicetype:data['jobTask'], status: data['status'], profile: data['profile'],),)),
            );
          }).toList(),
        );
      },
